...
| Start from no project through the generic Runtime | Typed `project.create` creates one non-existing HFSS 3D Layout bundle, saves and closes it, performs a fresh AEDT reopen, and returns an opaque, privately content-bound `EDA_CONTEXT`; context-driven `native.batch` materializes fixed source identity and returns a continuation Context | Validated on AEDT 2026.1.0 / PyAEDT 1.4.0 / Linux / `DISPLAY=:4.0`; continuation binding and conflict paths are synthetic-tested | HFSS 3D Layout only; refuses overwrite, does not solve, does not expose the remote project path or fingerprint in the token, and does not derive engineering scope or validation |
...
| Read live HFSS 3D Layout identity | Bounded PyAEDT live probe; optionally reuses an exact Runtime-owned session through its opaque resource id and handle | Validated on AEDT 2026.1.0 / PyAEDT 1.4.0 on Linux; cold and owned-session timing is recorded separately | A cold probe opens and closes only its own new session; reuse requires matching project, version, design, process, and active ownership and never selects an arbitrary running AEDT process |
| Make a small supervised change in the design currently visible in AEDT | `design.live_patch` and `design.live_finalize` reuse either an exact Runtime-owned session or the live project/design Context captured by the add-in; typed design-variable edits require an exact prior value and immediate readback | Validated on AEDT 2026.1.0 / PyAEDT 1.4.0 / Linux / `DISPLAY=:4.0` through Codex and Pi Agent | v1 changes design variables only. Context identity is revalidated on every call; it does not grant release authority, and save/discard is never implicit |
