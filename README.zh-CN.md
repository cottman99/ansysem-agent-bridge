...
## 小修改直接出现在你正在看的 AEDT 窗口里

![ADS 与 AEDT 受监督实时编辑的观测耗时](docs/assets/readme/supervised-live-edit-latency.png)

AEDT 2026 R1 验收复用了同一个已打开图形进程。一个类型化 Patch 在 `TOP`
层创建了命名的 `2 mm × 1 mm` 矩形并立即回读版图属性，观测 Adapter 时间为
**937 ms**；完全相同的回放用时 **12 ms**，重复对象为 0；带指纹检查、仅
针对该 Patch 的回滚用时 **204 ms**，并验证只有该矩形消失。暖态实时修改
完成于 **296–453 ms**。

这些是 2026-08-31 在可丢弃工程中对 `0.2.0a12` 的受控观察，不是普遍延迟
...
| “就在我正看着的版图里画这个矩形，但先不要保存。” | 复用 Context 指定的同一个 AEDT 图形进程，应用幂等类型化 Patch、立即回读对象，并可只回滚该 Patch。 |
...
2. 有人工监督的小修改复用当前图形会话，并明确选择保留、保存或放弃。
3. 无人值守或高风险工作才从冻结源创建一个任务候选工作区，并合并兼容 Patch。
4. 持久检查点必须全新重开后才能接受。
5. 只有用户要求交付时，才正式 Promotion 一次。

这既避免无边界地直接修改原工程，也避免每个小修正都创建永久版本。
...
- 默认公开接口不允许任意 Python；
- 受监督实时修改不隐式保存或放弃；无人值守持久修改保护源工程并要求全新重开；
- 候选尝试不创建永久交付版本；
